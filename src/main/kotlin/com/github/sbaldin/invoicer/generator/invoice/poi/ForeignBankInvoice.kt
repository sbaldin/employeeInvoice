            ...
            document = javaClass.getResourceAsStream("/invoice_foreign.xlsx").let {
                XSSFWorkbook(it).apply {
                    getSheetAt(0).apply {
                        ...
                        setRowValue(rowIndex++, 1, getDecimalFormatter().format(employeeDetails.monthRate))
                        setRowValue(rowIndex++, 1, 0) // employeeDetails.additionalExpenses
                        rowIndex++ // skipping rows
                        setRowValue(rowIndex++, 1, getDecimalFormatter().format(employeeDetails.monthRate)) // employeeDetails.additionalExpenses
